...
public function Invertido(){
	$sql=" SELECT articulo.idarticulo, articulo.nombre,articulo.stock, ROUND(articulo.PrecioCosto * articulo.stock,2) as costo, ROUND(articulo.PrecioVenta * articulo.stock,2) as venta from articulo order by articulo.idarticulo asc";
	return ejecutarConsulta($sql);
}


public function TotalDia($FechaInicio,$FechaFin){
	$sql=" select sum(venta.total_venta) as Total FROM venta WHERE venta.fecha_hora BETWEEN '".$FechaInicio."' AND '".$FechaFin."'";
	return ejecutarConsulta($sql);
}
