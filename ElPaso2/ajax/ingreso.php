...
switch ($_GET["op"]) {
	...
	case 'listarDetalle':
		...
		echo '</tbody><tfoot><tr>
			...
			<th></th>
			<th></th>
			<th><h4 id="totalModalIngreso">Q. '.$total.'</h4></th>
		</tr></tfoot>';
		break;
    ...
    case 'listar':
		...
		while ($reg=$rspta->fetch_object()) {
			$data[]=array(
            "0"=>($reg->estado=='Aceptado')?'<button class="btn btn-warning btn-xs" onclick="verDetalleIngreso('.$reg->idingreso.')"><i class="fa fa-eye"></i></button>'.' '.'<button class="btn btn-danger btn-xs" onclick="anular('.$reg->idingreso.')"><i class="fa fa-close"></i></button>':'<button class="btn btn-warning btn-xs" onclick="verDetalleIngreso('.$reg->idingreso.')"><i class="fa fa-eye"></i></button>',
            "1"=>$reg->fecha,
            "2"=>$reg->proveedor,
            "3"=>$reg->usuario,
